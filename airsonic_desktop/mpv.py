...
class MPV(object):
	...
	def register_stream_protocol(self, proto, open_fn):
		""" Register a custom stream protocol as documented in libmpv/stream_cb.h:
			...
			open_fn is a function taking an URI string and returning either an mpv stream object, or a tuple (size,
			read_generator) of the streams size (None if unknown) and a gnerator yielding bytes() chunks of read data.
			open_fn may return None to signal libmpv the URI could not be opened.

			The mpv stream protocol is as follows:
			class Stream:
				@property
				def size(self):
					return None # or int(size)
	...
	def _python_stream_open(self, uri):
		...
		"""
		name, = re.fullmatch('python://(.*)', uri.decode('utf-8')).groups()
		...
		if name in self._python_streams:
			...
			if self._python_stream_catchall is not None:
				...
				raise ValueError('Python stream name not found and not catch-all defined')
